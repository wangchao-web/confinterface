...
public abstract class TerminalService {
    ...
    public boolean updateExchange(Vector<MediaDescription> mediaDescriptions) {
        LogTools.info(LogOutputTypeEnum.LOG_OUTPUT_TYPE_FILE, "now in updateExchange, mediaDescriptions : " + mediaDescriptions.get(0).toString());
        System.out.println("now in updateExchange, mediaDescriptions : " + mediaDescriptions.get(0).toString());
        ...
        for (MediaDescription mediaDescription : mediaDescriptions) {
            if (baseSysConfig.isSendRecvPort()) {
                mediaDescription.setDirection(MediaDirectionEnum.SendRecv);
            }
            ...
            for (DetailMediaResouce detailMediaResouce : channel) {
                ...
                LogTools.info(LogOutputTypeEnum.LOG_OUTPUT_TYPE_FILE, "updateExchange,detailMediaResource(type:" + detailMediaResouce.getType() + ", dual:" + detailMediaResouce.getDual() + ", StreamIndex : " + detailMediaResouce.getStreamIndex() + "resourceID" + detailMediaResouce.getId() + ")");
                System.out.println(" updateExchange ,detailMediaResource(type:" + detailMediaResouce.getType() + ", dual:" + detailMediaResouce.getDual() + ", StreamIndex : " + detailMediaResouce.getStreamIndex() + "resourceID" + detailMediaResouce.getId() + ")");

                if (!mediaDescription.getMediaType().equals(detailMediaResouce.getType())) {
                    continue;
                }

                if (mediaDescription.getDirection() != MediaDirectionEnum.SendRecv && mediaDescription.getStreamIndex() != detailMediaResouce.getStreamIndex()) {
    ...
    public TerminalOfflineReasonEnum callMt(P2PCallParam p2PCallParam) {
        ...
        synchronized (this) {
            P2PVideoCallMediaCap videoCodec = p2PCallParam.getVideoCodec();
            P2PAudioCallMediaCap audioCodec = p2PCallParam.getAudioCodec();

            remoteMtAccount = account;

            TerminalOfflineReasonEnum terminalOfflineReasonEnum;
            terminalOfflineReasonEnum = callRemote(remoteParticipantInfo, videoCodec, audioCodec);
            ...
            if (TerminalOfflineReasonEnum.OK == terminalOfflineReasonEnum) {
    ...
    public P2PCallResult translateCall(String srcE164, ConfSessionPeer caller) {
        ...
        if (jsonObject.containsKey("videoCodec")) {
            ...
            P2PVideoCallMediaCap p2PCallMediaCap = (P2PVideoCallMediaCap) JSONObject.toBean(videoCodecObject, P2PVideoCallMediaCap.class);
    ...
    protected List<String> isNeedCreateResource(boolean bReverse, Vector<MediaDescription> mediaDescriptions) {
        ...
        for (MediaDescription mediaDescription : mediaDescriptions) {
            for (DetailMediaResouce detailMediaResouce : channel) {
                ...
                if (mediaDescription.getDual() && detailMediaResouce.getDual() != 1
                        || !mediaDescription.getDual() && detailMediaResouce.getDual() == 1)
                    continue;

                int streamIndex = mediaDescription.getStreamIndex();
        ...
        if (null == exchangeInfos) {
            ...
            for (String resourceId : resourceInfo) {
                for (DetailMediaResouce detailMediaResouce : channel) {
        ...
        if (!bOk) {
            removeMediaResource(bReverse, resourceInfo);
            updateResourceParams.clear();
            resourceInfo.clear();
            return resourceInfo;
        }

        updateMediaResource(bReverse, exchangeInfos);
    ...
    protected String constructCreateSdp(MediaDescription mediaDescription) {
        ...
        if (mediaDescription.getDirection() == MediaDirectionEnum.SendRecv) {
    ...
    protected MediaDescription constructRequestMediaDescription(MediaDescription mediaDescription, String exchangeSdp) {
        ...
        if (exchangeSdp.contains("sendonly")) {
            newMediaDescription.setDirection(MediaDirectionEnum.Send);
        } else if (exchangeSdp.contains("recvonly")) {
            newMediaDescription.setDirection(MediaDirectionEnum.Recv);
        } else {
            newMediaDescription.setDirection(MediaDirectionEnum.SendRecv);
        }

        TransportAddress rtpAddress = constructTransAddress(exchangeSdp);
        LogTools.info(LogOutputTypeEnum.LOG_OUTPUT_TYPE_FILE, "constructRequestMediaDescription rtpAddress : " + rtpAddress.toString());
        System.out.println("constructRequestMediaDescription rtpAddress : " + rtpAddress.toString());
        ...
        LogTools.info(LogOutputTypeEnum.LOG_OUTPUT_TYPE_FILE, "constructRequestMediaDescription newMediaDescription : : " + newMediaDescription.toString());
        System.out.println("constructRequestMediaDescription newMediaDescription : " + newMediaDescription.toString());
        return newMediaDescription;
    }

    protected static TransportAddress constructTransAddress(String sdp) {
        if (null == sdp) {
            return null;
        }
    ...
    protected static String constructSdp(MediaDescription mediaDescription) {
        ...
        StringBuilder sdp = new StringBuilder();
        TransportDirectionEnum direction = TransportDirectionEnum.SEND;

        do {
            if (mediaDescription.getDirection() == MediaDirectionEnum.SendRecv && direction == TransportDirectionEnum.SEND) {
                sdp.append("c=IN ");
                sdp.append(rtpProtocolType);
                sdp.append(" ");
                if ("IP4".equals(rtpProtocolType)) {
    ...
    protected void addMediaResource(int streamIndex, boolean dual, CreateResourceResponse resourceResponse) {
        ...
        LogTools.info(LogOutputTypeEnum.LOG_OUTPUT_TYPE_FILE, "addMediaResource exchangeSdp : " + resourceResponse.getSdp());
        System.out.println("addMediaResource exchangeSdp : " + resourceResponse.getSdp());
        ...
        if (resourceResponse.getSdp().contains("a=sendonly")
                || resourceResponse.getSdp().contains("a=sendrecv")) {
    ...
    protected Vector<MediaDescription> constructAckMediaDescription(Vector<MediaDescription> mcuOpenChannelMediaDescriptions) {
        ...
        for (MediaDescription mediaDescription : mcuOpenChannelMediaDescriptions) {
            ...
            for (DetailMediaResouce detailMediaResouce : channels) {
                ...
                if (MediaDirectionEnum.Send == mediaDescription.getDirection()) {
                    localMediaDescription.setDirection(MediaDirectionEnum.Recv);
                } else if (MediaDirectionEnum.Recv == mediaDescription.getDirection()) {
                    localMediaDescription.setDirection(MediaDirectionEnum.Send);
                } else {
                    localMediaDescription.setDirection(MediaDirectionEnum.SendRecv);
                }
    ...
    protected boolean ackOpenLogicalChannel(Vector<MediaDescription> mcuDescriptions) {
        ...
        LogTools.info(LogOutputTypeEnum.LOG_OUTPUT_TYPE_FILE, "participartId: " + e164 + ", ackOpenLogicalChannel, localMediaDescriptions.get(0): " + localMediaDescriptions.get(0).toString());
        System.out.println("participartId: " + e164 + ", ackOpenLogicalChannel, localMediaDescriptions.get(0): " + localMediaDescriptions.get(0).toString());
    ...
    protected void constructMediaCodec(P2PVideoCallMediaCap videoCallMediaCap, P2PAudioCallMediaCap audioCallMediaCap, MediaCodec mediaCodec) {
        mediaCodec.getVideoCapability().setBitrate(videoCallMediaCap.getBitrate());
        mediaCodec.getVideoCapability().setEncodingFormat(EncodingFormatEnum.FromName(videoCallMediaCap.getCodecFormat()));
        mediaCodec.getVideoCapability().setResolution(ResolutionEnum.fromName(videoCallMediaCap.getResolution()));
        mediaCodec.getVideoCapability().setFramerate(videoCallMediaCap.getFramerate());
    }
